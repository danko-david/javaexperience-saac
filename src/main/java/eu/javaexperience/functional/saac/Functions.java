...
/*	public static void collectFunctions(Collection<PreparedFunction> dst, Class... sources)
	{
		ArrayList<PreparedFunction> funcs = new ArrayList<PreparedFunction>()
		{
			@Override
			public boolean add(PreparedFunction e)
			{
				String id = e.id;
				for(PreparedFunction in:this)
				{
					if(in.id.equals(id))
					{
						return false;
		...
		try
		{
			...
			(
				funcs, Class.forName
				(
					"hu.futureweb.arukereso.site_spec_spider.JsoupFunctions"
				)
			);
		}
		catch(Exception e)
		...
		collectFunctions(funcs, SpiderFunctional.class);
		collectFunctions(funcs, GeneralFunctions.class);
		collectFunctions(funcs, StringFunctions.class);
		collectFunctions(funcs, ImapFunctions.class);
		collectFunctions(funcs, BoolFunctions.class);
		
		collectFunctions(funcs, CorFunctions.class);
		collectFunctions(funcs, ImapTools.class);
		
		collectFunctions(funcs, ImapSorterTools.class);
		collectFunctions(funcs, DispatcherFunctions.class);
		
		collectFunctions(funcs, SaacFunctions.class);
		
		
		
		//collectFunctions(funcs, );
		
		Map<String, PreparedFunction> index = new HashMap<>();
		MapTools.extractedIndexFirstOnly(index, funcs, new GetBy1<String, PreparedFunction>()
		{
			@Override
			public String getBy(PreparedFunction a)
			{
				return a.getId();
			}
		});
